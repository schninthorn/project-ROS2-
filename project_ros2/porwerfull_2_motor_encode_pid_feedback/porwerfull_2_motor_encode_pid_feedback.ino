...
rcl_publisher_t    feedback_vel_pub;   // +++ เพิ่ม
rcl_subscription_t target_left_sub;
rcl_subscription_t target_right_sub;
rcl_subscription_t pid_gain_sub;       // +++ เพิ่ม
...
geometry_msgs__msg__Twist       feedback_vel_msg;  // +++ เพิ่ม
geometry_msgs__msg__Vector3     pid_gain_msg;      // +++ เพิ่ม
...
// ===== min / max helper =====  // +++ เพิ่ม
float myMax(float a, float b) { return (a > b) ? a : b; }
float myMin(float a, float b) { return (a > b) ? b : a; }
...
void pid_timer_callback(rcl_timer_t * timer, int64_t last_call_time) {
  ...
  // --- counter_tick & counter_RPM ---  // +++ เพิ่ม
...
void create_entities() {
  ...
  rclc_publisher_init_default(&feedback_vel_pub, &node,   // +++ เพิ่ม
  ...
  rclc_subscription_init_default(&pid_gain_sub, &node,    // +++ เพิ่ม
  ...
  // Executor: 1 timer + 3 subscribers  // +++ เพิ่มจาก 3 -> 4
  ...
  rclc_executor_add_subscription(&executor, &pid_gain_sub,  // +++ เพิ่ม
    &pid_gain_msg, &pid_gain_callback, ON_NEW_DATA);
}
...
void destroy_entities() {
  ...
  rcl_publisher_fini(&feedback_vel_pub,  &node);  // +++ เพิ่ม
  rcl_subscription_fini(&target_left_sub,  &node);
  rcl_subscription_fini(&target_right_sub, &node);
  rcl_subscription_fini(&pid_gain_sub,     &node);  // +++ เพิ่ม
